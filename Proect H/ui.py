...
def draw_text(surface, text, pos, color, font=FONTS["medium"], pos_mode="top_left",
    ...
    if shadow: # יוצר את הצלליות של הטקסט
...
def button(surface, pos_y, text=None, click_sound=None):
    ...
    pygame.draw.rect(surface, COLORS["buttons"]["shadow"], (rect.x - 6, rect.y - 6, rect.w, rect.h)) # ציור הצללית כפתורים
    ...
    if on_button and pygame.mouse.get_pressed()[0]: # לחיצה על העכבר
        if click_sound is not None: # מנגן מוזיקה של לחיצה
            click_sound.play()
        return True
